    ...
    with sqlite3.connect('database.db') as conn:
        ...
        if 'email' not in session:
            ...
            cur.execute("SELECT userId, firstName FROM users WHERE email = '" + session[
                'email'] + "'")  # Elmi A. changed user_id & first_name to userId & firstName

            user_id, first_name = cur.fetchone()
            cur.execute("SELECT count(productId) FROM kart WHERE userId = " + str(user_id))  # Elmi A. changed user_id to userId
...
def root():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        # Show all categories
        cur.execute('SELECT categoryId, name FROM categories')  # Tim N. removed hashtag in front of this line for category fix
        category_data = cur.fetchall()  # Tim N. removed hashtag in front of this line for category fix
...
def edit_profile():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT userId, email, first_name, lastName, address1, address2, zipcode, city, state, country, phone FROM users WHERE email = '" +
            session['email'] + "'")
        profile_data = cur.fetchone()
    conn.close()
    return render_template("editProfile.html", profileData=profile_data, loggedIn=logged_in, firstName=first_name,
